...
const validateRouteOptions = (optionsDirty: mixed): RouteOptions => {
  ...
  if (validate) {
    // report extraneous keys
    Object.keys(validate).forEach((key) => {
      if (key !== 'body' && key !== 'query' && key !== 'params') {
        throw new Error('options.validate can only have the following keys: ' +
        'body, query, params');
      }
    });
